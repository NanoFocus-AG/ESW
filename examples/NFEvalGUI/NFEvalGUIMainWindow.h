...
public:
  ...
  virtual void newLogMessage(int LogLevel, const char *context, const char *message, const NFLogLocationInfo &loc,
                             long long timeStamp, long long timeAfterStart);
public slots:
...
protected Q_SLOTS:
  ...
  void readSettings();
  void writeSettings();
  void mousePositionSlot(double xPos, double yPos, double zPos, unsigned int colX, unsigned int rowY, double heightVal,
                         int intensityVal, unsigned int maskVal, int colorVal);
...
class NFEvaluationThread : public QThread
{
  Q_OBJECT
public:
  NFEvaluationThread() : m_ResultStatus(0){};
